...
def main(args=None):
    ...
    pkg_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'rl_models')
    trained_model_path = os.path.join(pkg_dir, 'PPO_second_generation.zip')
    node.get_logger().info(f"Loading model: {trained_model_path}")
    register(id="PharmaBotEnv-v0",
             entry_point="hospital_robot_spawner.pharmabot_env:PharmaBotEnv",
             max_episode_steps=3000)
    env = Monitor(gym.make('PharmaBotEnv-v0'))
    check_env(env)
    co = {'action_space': env.action_space, 'observation_space': env.observation_space}
    model = PPO.load(trained_model_path, env=env, custom_objects=co)
