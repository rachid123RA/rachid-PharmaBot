...
class MissionManager(Node):
    ...
    def _cb_livraison_confirmee(self, msg: String):
        ...
        CORRECTIF BUG S06 :
          - Vérifie mission courante AVANT d'accepter la confirmation.
          - Vérifie correspondance département livré == département mission.
          - Vérifie état machine compatible (ignore les confirmations orphelines).
        """
        try:
            data = json.loads(msg.data)
            ...
            # Garde 1 : mission courante doit exister
            if not self._mission_courante:
                self.get_logger().warn(
                    f"[MISSION MGR] Livraison confirmée ({dept}) "
                    "mais aucune mission courante — ignoré")
                return

            # Garde 2 : département doit correspondre
            if self._mission_courante.get("departement") != dept:
                self.get_logger().warn(
                    f"[MISSION MGR] Livraison {dept} ≠ mission "
                    f"{self._mission_courante.get('departement')} — ignoré")
                return

            # Garde 3 : état machine compatible
            etats_ok = ("NAVIGATION", "LIVRAISON", "URGENCE_OVERRIDE", "CHARGEMENT")
            if self._etat not in etats_ok:
                self.get_logger().warn(
                    f"[MISSION MGR] État={self._etat} non compatible "
                    "avec fin de livraison — ignoré")
                return
